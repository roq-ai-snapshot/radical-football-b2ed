...
import { deletePlayerParentById } from 'apiSdk/player-parents';
...
function PlayerViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PlayerInterface>(
    ...
    () =>
      getPlayerById(id, {
        relations: ['user', 'academy', 'player_note', 'player_parent'],
      }),
  );
  ...
  const player_parentHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deletePlayerParentById(id);
      await mutate();
    } catch (error) {
      setDeleteError(error);
    }
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('player_parent', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Player Parents
                    </Text>
                    <NextLink passHref href={`/player-parents/create?player_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.player_parent?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/player-parents/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('player_parent', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink passHref href={`/player-parents/edit/${record.id}`}>
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                                </NextLink>
                              )}
                              {hasAccess('player_parent', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={() => player_parentHandleDelete(record.id)}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
                                  icon={<FiTrash />}
